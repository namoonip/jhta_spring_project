...
<body>
	...
	<div class="container" style="margin-left: 250px; padding-top:25px; ">
		<div class="row text-right">
			홈 > 학적관리 > 자퇴관리 > <strong>자퇴 승인 목록</strong> 
		</div>
		<div class="row" style="margin: 0px; padding: 0px;">
			<h4><span class="glyphicon glyphicon-list-alt"></span> 자퇴 승인 목록</h4>
						...
						<table class="table table-striped table-bordered table-hover" id="dataTables-example">
							...
							<tbody>
								...
								<c:forEach var="drop" items="${dropList }" varStatus="status">
									<tr onclick="location.href='dropinfoOK?no=${drop.no}'" style="cursor: pointer;">
										<td>${status.count }</td>
										<td>${drop.code }</td>
										<td>${drop.student.id }</td>
										<td>${drop.student.name }</td>
										<td>${drop.student.division }</td>
										<td><fmt:formatDate value="${drop.passDate }" pattern="yyyy-MM-dd"/></td>
										<td class="text-center">
											<c:choose>
												<c:when test="${drop.pass == 'ok'}"><font color="blue"><strong>승인</strong></font></c:when>
		...
		<form action="enterdropoff" method="post" id="search-form">
