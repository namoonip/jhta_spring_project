   ...
   <div class="container" style="margin-left: 250px; padding-top:25px; ">
      	...
      	<table class="table table-bordered">
			...
			<tbody>
				<tr>
					...
					<th>
						<fmt:formatDate value="${leaves.reinDate }" pattern="YYYY-MM-dd"/></th>
					<th>
						<c:choose>
							<c:when test="${leaves.pass eq 'false' }">
								<input type="hidden" id="leave-Pass" value="false" />
								<font color="red"><strong>미승인</strong></font>
							</c:when>
							<c:when test="${leaves.pass eq 'ok' }">
								<input type="hidden" id="leave-Pass" value="false" />
      ...
      <div class="row" style="padding: 20px;">
      	...
      	<table class="table table-bordered">
			...
			<tbody>
				<tr>
					<th><fmt:formatDate value="${rein.reinDate }" pattern="yyyy"/>년</th>
					<th>
						<fmt:formatDate value="${rein.reinDate }" pattern="MM" var="seme"/>
						<c:if test="${seme > 6 }">
							2
						</c:if>
						<c:if test="${seme < 6 }">
					...
					<th>
						<fmt:formatDate value="${rein.reinDate }" pattern="YYYY-MM-dd"/></th>	
					<th>
						<a class="btn btn-sm btn-primary" href="reinOK?no=${rein.no }">승인</a>
					</th>
				</tr>
			</tbody>      	
      	</table>
